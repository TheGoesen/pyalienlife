...
local function max_value(outpost_inventory)
    local maxCount, type
    for item_type, count in pairs(outpost_inventory.get_contents()) do
        if maxCount == nil or count > maxCount then
            type = item_type, maxCount
        end
    end
    return type

end
Caravan.parameters_for_action = function(action, caravan_data, schedule_id)
    ...
    if action.type == 'item-count' then
        ...
        local outpost_inventory = Caravan.get_inventory(chest)
        if not outpost_inventory then return false end
        local type = max_value(outpost_inventory)
        if not type then return end
        action.elem_value = type
        action.item_count = game.item_prototypes[type].stack_size
...
gui_events[defines.events.on_gui_selection_state_changed]['py_add_action'] = function(event)
    ...
    Caravan.parameters_for_action(action, caravan_data, element.tags.schedule_id)
